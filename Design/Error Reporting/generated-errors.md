...
information during codegen phase. A **error context information** is an extra piece of info added to a generated Viper's
...
| ---------------------------- | --------------------------------------------------------------- | ------------------- | -------------------------------------------- | ------------------------------------------------------------------------------------- |
| `ListIndexAccess`            | Accessing a list element with the index operator.               | * The list's size.  | `list.kt`, `binary_search.kt`                | `warning: indexing the list with the provided index may result in a runtime exception.` |
| `SublistOutOfRange`          | Creating a sublist with two wrong interval ranges.              | * The list's size.  | `binary_search.kt`                           | `warning: the interval indices are not contained between the list's size.`            |
| `LeakingLambdaParameter`     | Passing a lambda parameter targeted by a `callsInPlace` effect. | -                   | `calls_in_place_leak.kt`                     | `warning: the following lambda parameter may leak outside this function.`             |
| `TypeInferenceMismatch`      | Implication on returning type of a conditional contract.        | -                   | `as_type_contract.kt`, `is_type_contract.kt` | `warning: the returned type might be different from the stated one.`                  |
| `ConditionalEffectViolation` | Conditional effect given by an implication.                     | -                   | `returns_boolean.kt`, `returns_null.kt`      | `warning: the implication's consequence might not hold.`                              |
...
2.  Keeping an error context collector during the codegen, and insert unique errors, while we generate AST Viper's Node,
...
AST node. Embedding this information on a Viper's AST Node metadata would be similar to the work we have done with
positions (i.e., create a wrapper around the context information we want to save). Using the table defined above, we can 
define a error context information as it follows.

```kotlin
sealed class ErrorContextInfo {
    object ListIndexAccess              : ErrorContextInfo()
    object SublistOutOfRange            : ErrorContextInfo()
    object LeakingLambdaParameter       : ErrorContextInfo()
    object TypeInferenceMismatch        : ErrorContextInfo()
    object ConditionalEffectViolation   : ErrorContextInfo()
} 
```
...
1.  Creating a *context error information* collector when verifying a function. This data structure would behave
    as a central point used during the error reporting phase. Its scope will be clearer as follows.
2.  Instead of embedding the *context error information* on a Viper's AST node, we use a collector's instance
...
3.  During the error-reporting phase, since each `VerificationError` has an `offendingNode`, we can retrieve
    the identifier and use it to get the context error information. Then, having the Viper's error and the context
    we can output a warning accordingly.

The main difference between the two approaches is how they keep the context error information. The second approach
...
Both approaches allow to concatenate context information. For example, if we want to give an advanced error's 
explaination to the user for `ListAccessing`, we can store the list's size. So, the message can become like the
following:
