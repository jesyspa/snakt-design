...
*   Accessing list without of bound indices (`binary_search.kt`, `list.kt`)
...
Every Viper’s Verification error has two pieces of information along with them: a string identifier and 
...
Each one of these errors has fields bringing extra information, illustrated in the table below. All axes are explained
accordingly:

1.  Error Kind: what type of error Viper generated (violation of post-condition, failed precondition, failed assertion).
2.  Reason: it informs the user why the error generated. For violation of (pre/post)condition is represented by the
    ...
    a verification error is located.
3.  Offending Node: in which Viper’s AST node the error is located at.
4.  Extra Fields: other fields carried by the error.
5.  Context: what we were verifying when the error shown up.
6.  What went wrong: user-friendly reason of why the verification failed.
...
*   The `Postconditionviolated` error is the only one carrying extra information (i.e., `Method`) pointing out
    in which method the violated postcondition is located.
...
to the FFI with the JVM (Silicon is written in Scala), they adopted some tricks discussed later in the section.
...
information* during codegen phase. A **source’s role information** is an extra piece of info added to a generated Viper’s
AST node. The kind of information can be related to *what* we are analyzing. For example, if we are generating Viper’s
code for Kotlin’s lists access, we can embed an information on what could go wrong when a `PreconditionInCallFalse` is
...
| SourceRole Information             | When embed this information                                     | Additional Metadata | Example File                                 | Possible Error Message                                                                  |
| ---------------------------------- | --------------------------------------------------------------- | ------------------- | -------------------------------------------- | --------------------------------------------------------------------------------------- |
...
| `Returns(True\False\Null\NotNull)` | Returns effect on a Kotlin’s contract.                          | -                   | `returns_boolean.kt`, `returns_null.kt`      | `warning: the function may return a {0} value.` |
| `CondEffect`                       | Conditional effect given by an implication.                     | -                   | `returns_null.kt`                            | `warning: the implication's consequence might not hold.`                                |
...
The first way, consists in defining a new source’s role information to be embedded as `Info` data structure into a Viper’s
...
define a source’s role as it follows.
...
Having the `SourceRole`, we can create a new Viper’s Info sub-child wrapping the information and embed it
...
According to Prusti’s team, this solution came up from using Viper with another programming language different from a
...
1.  Creating a *source’s roles* collector when verifying a function. This data structure would behave
...
3.  During the error-reporting phase, since each `VerificationError` has an `offendingNode`, we can retrieve
    the identifier and use it to get the source’s role. Then, having the Viper’s error and the context
    we can output a warning accordingly.

The main difference between the two approaches is how they keep the source’s role information. The second approach
...
This section illustrates where on what nodes a source’s role information should be embedded.
...
a source’s role information on the method calls representing the Kotlin list function. So, for (1) we
embed the info `IndexInBoundCondition`, and for (2) the `SublistInBoundCondition`.
...
call to the `duplicale` built-in function. In this case, we can embed the `DublicableLambda` source’s role 
...
form `Implies` (left → right), then we can embed a specific source’s role information on the right implication side.
In the case we are dealing with `returns()` clause without right side, we embed the info directly on the corresponding
Viper’s AST Node.

For example:
*   `bad_contracts/is_type_contract.kt`. We can embed a source’s role information of type `IsSubType` on 
